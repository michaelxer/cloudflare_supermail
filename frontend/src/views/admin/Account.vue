...
// Bulk bind to Telegram
const showBulkBindModal = ref(false)
const bulkBindUserId = ref('771709504')
const bulkBindLoading = ref(false)
const bulkBindResult = ref(null)

const bulkBindTelegram = async () => {
    if (!bulkBindUserId.value) {
        message.error('Please enter a Telegram User ID')
        return
    }
    try {
        bulkBindLoading.value = true
        const res = await api.fetch('/admin/bulk_bind_telegram', {
            method: 'POST',
            body: JSON.stringify({ userId: bulkBindUserId.value })
        })
        bulkBindResult.value = res
        message.success(`Bound ${res.total} new addresses to Telegram`)
    } catch (error) {
        message.error(error.message || 'error')
    } finally {
        bulkBindLoading.value = false
    }
}
...
<template>
    <div style="margin-top: 10px;">
        ...
        <n-space style="margin-bottom: 10px;" align="center">
            ...
            <n-button type="info" tertiary @click="showBulkBindModal = true">
                Bulk Bind to Telegram
            </n-button>
        </n-space>

        <!-- Bulk Bind Telegram Modal -->
        <n-modal v-model:show="showBulkBindModal" preset="card" title="Bulk Bind All Addresses to Telegram" style="width: 500px;">
            <n-space vertical>
                <p style="opacity: 0.75; margin: 0;">This will bind ALL addresses in the system to your Telegram account. Already-bound addresses will be skipped.</p>
                <n-form-item label="Your Telegram User ID">
                    <n-input v-model:value="bulkBindUserId" placeholder="e.g. 771709504" />
                </n-form-item>
                <n-button type="primary" block :loading="bulkBindLoading" @click="bulkBindTelegram">
                    Bind All Unbound Addresses
                </n-button>
                <div v-if="bulkBindResult">
                    <n-tag type="success">Newly bound: {{ bulkBindResult.bound?.length || 0 }}</n-tag>
                    <n-tag type="default" style="margin-left: 8px;">Already bound / skipped: {{ bulkBindResult.skipped?.length || 0 }}</n-tag>
                </div>
            </n-space>
        </n-modal>
